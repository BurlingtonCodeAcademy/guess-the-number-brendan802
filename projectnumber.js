...
let rangeQuestion = await ask("Would you like to extend the number range by giving a outermost range number?")
    ...
    if (extendedRange > 1) { maxValue = extendedRange}
...
let question = await ask(`Let's play a game, are you thinking of a number between ${minValue} and ${maxValue}? `)
    while (question === 'no') {
    ...
    let first = await ask(`Is your number greater, less than , or equal to ${guess} ? `)
        ...
        //if number is higher 
            ...
            //cheat detector logic, if player does not admit when computer has guessed their number
                ...
                response= await ask (" You blasphemous cheater!!! Come back when you want to play fair!")
                process.exit()
            }
        }
    }
